#ifndef SERVICE_FRAMEWORK_TEST4_FRAMEWORK_HOST_SERVICEHOSTPROXY_HPP
#define SERVICE_FRAMEWORK_TEST4_FRAMEWORK_HOST_SERVICEHOSTPROXY_HPP
//****************************************************************************************************************************************************
...
namespace Test4
{
  ...
  /// to the target executor and return std::future for the result.
  ///
  /// Callbacks are provided via CompletionCallback, a moveable type-erased wrapper
  /// that completely hides implementation details. Use FutureCallbackHelper to
  /// create callbacks with proper lifetime tracking.
  ...
  public:
    ...
    /// @param callback Optional callback invoked when complete. Pass default-constructed for no callback.
    /// @return A future that completes when all services have been started.
    ...
    std::future<void> TryStartServicesAsync(std::vector<Test2::StartServiceRecord> services, const Test2::ServiceLaunchPriority currentPriority,
    ...
    /// @param callback Optional callback invoked when complete. Pass default-constructed for no callback.
    /// @return A future yielding a vector of exception_ptr for services that failed to shut down.
    ...
    ///       executes on the service host's executor.
    std::future<std::vector<std::exception_ptr>> TryShutdownServicesAsync(const Test2::ServiceLaunchPriority priority,
    ...
    ///
    /// @param callback Optional callback invoked when complete. Pass default-constructed for no callback.
    /// @return A future yielding true if the shutdown request was successfully
    ///         processed by the service host, or false if the service host is no longer
    ///         available.
    /// @note This method is safe to call from any thread. The actual shutdown logic
    ///       executes on the service host's executor.
    std::future<bool> TryRequestShutdownAsync(CompletionCallback<bool> callback = {});
