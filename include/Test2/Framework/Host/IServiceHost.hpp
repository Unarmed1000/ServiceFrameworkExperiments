#ifndef SERVICE_FRAMEWORK_TEST2_FRAMEWORK_HOST_ISERVICEHOST_HPP
#define SERVICE_FRAMEWORK_TEST2_FRAMEWORK_HOST_ISERVICEHOST_HPP
//****************************************************************************************************************************************************
...
namespace Test2
{
  class IServiceHost : public IService
  {
  public:
    ...
    IServiceHost(IServiceHost&&) = delete;
    IServiceHost& operator=(IServiceHost&&) = delete;
    ...
    virtual boost::asio::awaitable<std::vector<StartedServiceInfo>> TryStartServicesAsync(std::vector<StartServiceRecord> services,
                                                                                          const ServiceLaunchPriority currentPriority) = 0;

    /// @brief Try to start service proxies at a given priority level.
    ///
    /// This will be called after TryStartServicesAsync to start proxies for the services (of the given priority).
    ///
    /// This method can be called from any thread. The work is marshalled onto the
    /// service thread via co_spawn, ensuring thread-safe access to internal state.
    ///
    /// Service proxies are created, initialized, and registered with the provider.
    /// On failure, successfully initialized services are rolled back.
    ///
    /// @param services Services to start.
    /// @param currentPriority Priority level for this group.
    /// @return Awaitable that completes when proxies are started.
    ...
    /// @brief Shutdown service proxies at a specific priority level.
    ///
    /// This will be called before TryShutdownServicesAsync to stop proxies for the services (of the given priority).
    ///
    /// This method can be called from any thread. The work is marshalled onto the
    /// service thread via co_spawn, ensuring thread-safe access to internal state.
    ///
    /// @param priority The priority level to shut down.
    /// @return Awaitable containing any exceptions that occurred during shutdown.
    virtual boost::asio::awaitable<std::vector<std::exception_ptr>> TryShutdownServiceProxiesAsync(const ServiceLaunchPriority priority) = 0;
  };
}
