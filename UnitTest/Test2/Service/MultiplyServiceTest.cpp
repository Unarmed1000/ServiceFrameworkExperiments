...
namespace Test2
{
  ...
  // ============================================================================
  // MultiplyService Test Fixture
  // ============================================================================

  class MultiplyServiceTest : public ::testing::Test
  {
  protected:
  ...
  // ============================================================================
  // MultiplyService Lifecycle Tests
  // ============================================================================

  TEST_F(MultiplyServiceTest, UpdateProcessesWithoutBlocking)
  {
  ...
  TEST_F(MultiplyServiceTest, MultipleUpdatesSucceed)
  {
  ...
  TEST_F(MultiplyServiceTest, MultiplyAsyncReturnsCorrectResult)
  {
    // Get the Multiply service proxy
    auto serviceProvider = manager->GetServiceProvider();
    auto multiplyService = serviceProvider.TryGetService<IMultiplyService>();
    ASSERT_NE(multiplyService, nullptr);

    // Call the async method
    auto future = boost::asio::co_spawn(manager->GetExecutor(), multiplyService->MultiplyAsync(10.0, 3.0), boost::asio::use_future);
  ...
  TEST_F(MultiplyServiceTest, MultiplyAsyncHandlesMultipleOperations)
  {
    // Get the Multiply service proxy
    auto serviceProvider = manager->GetServiceProvider();
    auto multiplyService = serviceProvider.TryGetService<IMultiplyService>();
    ...
    auto future = boost::asio::co_spawn(
      ...
      [&]() -> boost::asio::awaitable<std::vector<double>>
      {
        ...
        results.push_back(co_await multiplyService->MultiplyAsync(20.0, 5.0));
        results.push_back(co_await multiplyService->MultiplyAsync(100.0, 25.0));
        results.push_back(co_await multiplyService->MultiplyAsync(-5.0, -15.0));
        results.push_back(co_await multiplyService->MultiplyAsync(3.14, 2.14));
        co_return results;
      }(),
    ...
    EXPECT_DOUBLE_EQ(results[1], 2500.0);
    EXPECT_DOUBLE_EQ(results[2], 75.0);
    EXPECT_NEAR(results[3], 6.7196, 0.001);
  }
}
