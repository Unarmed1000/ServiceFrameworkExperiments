...
|---------|-------------|-------------|------------|
| **Simple Request/Response** | One request → one response | `std::future<T>` / `std::async` | `Task<T>` / `async`-`await` |
| **Continuation / Callback** | Response → chained handling | `future.then(...)` (library) | `Task.ContinueWith(...)` |
...
| Pattern | Description | C++ Example | C# Example |
|---------|-------------|-------------|------------|
| **Event / Pub-Sub** | Broadcast → many responses | Callback lists, Qt signals/slots | `event EventHandler<T>`, `IObservable<T>` |
...
|--------------|---------------------------|-------------|-------|
| **Free-threaded** | ✅ Yes | Any thread (thread-safe) | Thread-safe, Concurrent |
| **Single-threaded** | ❌ No | Specific thread only | Thread-affine, STA, UI-thread-only |
| **Marshaled** | ✅ Yes (calls redirected) | Host thread | Dispatcher-based, Actor-style |
...
UpdateUI(result);  // ✅ Automatically back on UI thread
```
...
**Problem**: Change `AddService.hpp` → rebuild `CalculatorService` → rebuild all consumers
...
**Benefit**: Change `AddService` implementation → only rebuild `AddService.cpp`
...
sequenceDiagram
    ...
    IO->>Host: TriggerWake()
    Host->>QtLoop: WakeCallback → QMetaObject::invokeMethod

    QtLoop->>Host: Update()
    Host->>IO: Poll()
    IO->>Svc: Execute queued handlers

    Note over QtLoop: ✅ All service code runs on UI thread
```

---

# Pattern → Framework Mapping
...
# Appendix A3: Custom ui_thread_awaiter (💡 Idea)
